...
c
        write (nout,*)
        write (nout,*)
     &   'Weiland-Nordman eigenvalue equations, subroutine etaw16'
        write (nout,*)
c
...
c
     ...
     &     ' Five eqns for e phi/T_e, T_H, n_H, T_e, and Vp'
        ...
        elseif ( ieq .eq. 2 ) then
          ...
          write (nout,*)
          write (nout,*) ieq,' = ieq in sbrtn etaw16'
          call abortb(nout
     &     ,'the value of ieq is wrong in sbrtn etaw16')
        endif
c
        inital = .false.
c
...
c
     ...
     & ,' neq .lt. 2 in sbrtn etaw16')
c
      if ( ndim .gt. idim ) call abortb (nout
     & ,' ndim .gt. idim in sbrtn etaw16')
c
c
c..initialize arrays
c
...
cpis      write(nout,109) 'zgns= ', zgns     
c
      if ( zfns .lt. zepsqrt) zgns = 0
c
cpis      write (nout,109) 'gnsin', zgns
...
c
c%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c..save copy of matrix which is over-written by sbrtn f02gjf
c
      do j2=1,ieq
        do j1=1,ieq
...
c
c..force complex computation
c
      if ( letain(10) .eq. 0 ) lcomplex = .true.
      if ( letain(10) .lt. 0 ) lcomplex = .false.
c      lcomplex = .true.
c
c      if ( letain(6) .gt. 1 ) then
c
c..use IMSL eigenvalue routines (not currently used)
c
c        if ( lcomplex ) then
c
...
c          do j1=1,ieq
c            do j2=1,ieq
c              zamc(j1,j2) = cmplx ( zamr(j1,j2), zami(j1,j2) )
c              zbmc(j1,j2) = cmplx ( zbmr(j1,j2), zbmi(j1,j2) )
c            enddo
c          enddo
c
c          call gvccg ( ieq, zamc, idim, zbmc, idim, zalpha, zbeta
c     &      , zevec, idim )
c
c        else
c
c..find the eigenvalues and eigenvectors using IMSL routine gvcrg
c  using real matricies
c
c          call gvcrg ( ieq, zamr, idim, zbmr, idim, zalpha, zbeta
c     &      , zevec, idim )
c
c          perform(jd) = gpirg ( ieq, ieq, zamr, idim, zbmr, idim
c     &      , zalpha, zbeta, zevec, idim )
c
c          if ( lprint .gt. 1 ) then
c            write (nout,*)
c            write (nout,*)
c     &       ' Eigenvalues computed using IMSL routine gvcrg'
c            write (nout,*)
c            write (nout,*)
c     &       perform(jd),' = performance index from IMSL gpirg'
c          endif
c
c        ifail = 0
c
c        endif
c
c..compute the complex eigenvalues from results of IMSL routines
c
c        do j=1,ieq
c          ztemp1 = zbeta(j)
c          if ( abs(zbeta(j)) .lt. zepsqrt ) ztemp1 = zepsqrt
c          zomega(j) = real  ( zalpha(j) ) / ztemp1
c          zgamma(j) = aimag ( zalpha(j) ) / ztemp1
...
c         write (nout,*)
c         write (nout,*)
c    &     ' Eigenvalues computed using complex NAG routine f02gjf'
c
        else
c
c  use NAG routine for real matricies
c
          call f02bjf ( ieq, zamr, idim, zbmr, idim, ztol
     &     , zalfr, zalfi, zbeta, lmatv, zeigenv, idim, iter, ifail )
c
c         if ( ifail .gt. 0 ) call abortb ( nout
c     &     ,'ifail .gt. 0 after call f02bjf in sbrtn etaw16')
c
          write (nout,*)
          write (nout,*)
     &     ' Eigenvalues computed using real NAG routine f02bjf'
c
        endif
c
        nerr = ifail
...
c
        do j2=1,ieq
          zevec(j1,j2) = cmplx( 0.0, 0.0 )
          if ( abs(zomega(j2)) .lt. zepsqrt ) then
            do j1=1,ieq
              zevec(j1,j2) = cmplx( zeigenv(j2,j1), 0.0 )
            enddo
          elseif ( abs( zomega(j2+1) - zomega(j2) ) .lt. zepsqrt ) then
            do j1=1,ieq
              zevec(j1,j2) = cmplx( zeigenv(j1,j2), zeigenv(j1,j2+1) )
            enddo
          elseif ( abs( zomega(j2-1) - zomega(j2) ) .lt. zepsqrt ) then
            do j1=1,ieq
              zevec(j1,j2) = cmplx( zeigenv(j1,j2-1), -zeigenv(j1,j2) )
            enddo
          endif
        enddo
c
...
c
     ...
     & ,'ifail .gt. 0 after call f02gjf in sbrtn etaw16')
c
c
...
c
        write (nout,*)
        write (nout,*) '--------------------------------------'
     &    ,'  Final results from sbrtn etaw16'
c
        write (nout,*)
