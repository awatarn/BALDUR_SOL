...
c             = 11 for sbrtn mmm2001no anomalous transport in SOL
...
c
           zrmajorm = zrmajor(1)
           zrminorm = zrminor(mzones)
c           zrmajorm   = max (rtmajb, 0.01)
c           zrminorm   = max (rtminb, 0.01) 
...
c           zrmajorm   = max (rtmajb, 0.01)
c           zrminorm   = max (rtminb, 0.01)
c           write(6,*) 'radius= ',zrmajorm,' ',zrminorm,' ',rtmajb         
c           call exit(0) 
           zbtorm     = max (bzs * 1.0E-4, 0.01)
           zhydmass  = max (ahmean(1,mzones), 1.00)
c
...
c
c
c  Determine whether the plasma is in L or H mode. 
c         
           ztime = 0.5 * ( tai + tbi ) * uiet     
c
           if (((cbound(2)>epslon) .and. (nstep>50)) 
...
c
c PUB Nov 16, 2015. add the following option for calculation in 
c SOL with no anomalous transport
c
c..call routine mmm2001
c
        else if ( lthery(21) .eq. 11 ) then
c
c..set switches
c
          i1 = 3
...
c          write(*,*) i1, ipoints, iedge, isep, mzones
c          stop 10
c
          lsuper = lthery(22)
          lreset = 1
...
c
     ...
     &      + xiemps(jz))*usil**2
cpub1     
c The actual values must be set in dtrans.f
c This part is just printout, not effect any calculation.
c	  write(*,*) iscrp1, iscrp2, iscrp3
c	  write(*,*) cfutz(iscrp1),cfutz(iscrp2),cfutz(iscrp3)
c	  write(*,*) epslon
c	  stop 99
	  if(jz .ge. isep) then
	    if(cfutz(iscrp3).gt.1234566.and.cfutz(iscrp3).lt.1234568) then
c           set thermal conductivities for electron and ion to be equal to 
c           neoclassical transport coefficient
	      zthibohm(jz)=0.0
	      zthigb(jz)=0.0
	      zthimix(jz)=0.0
	      xiemps(jz)=0.0
	      ...
	      zthikb(jz)=0.0
	      zxithe(jz)=.0
	      zxitot(jz)=cfutz(12)*xineo1(jz)*usil**2
	      ...
	      zthetb(jz)=0.0
	      zxethe(jz)=0.0
	      zxetot(jz)=(cfutz(11)*xeneo1(jz)+xeneo2(jz))*usil**2	    
	    else if(cfutz(iscrp3).gt.epslon) then
c	      constant chi_i_sol and chi_e_sol	    
c	      2*chi_i_sol=chi_e_sol=cfutz(iscrp3)/1.e4
	      zthibohm(jz)=0.0
	      zthigb(jz)=0.0
	      zthimix(jz)=0.0
	      xiemps(jz)=0.0
	      ...
	      zthikb(jz)=0.0
	      zxithe(jz)=.0
	      zxitot(jz)=0.5*cfutz(iscrp3)/1.e4
	      ...
	      zthetb(jz)=0.0
	      zxethe(jz)=0.0
	      zxetot(jz)=cfutz(iscrp3)/1.e4
	    endif

	    if(cfutz(iscrp1).gt.epslon) then
c	      set the hydrogen-ion diff coeff to be cfutz(iscrp1)/1.e4
	      zthdig(jz)=0.0
	      zthdrb(jz)=0.0
	      zthdkb(jz)=0.0
	      zdhthe(jz)=cfutz(iscrp1)/1.e4
	    endif
	    if(cfutz(iscrp2).gt.epslon) then
c	      set the hydrogen-ion diff coeff to be cfutz(iscrp1)/1.e4
	      zthzmix(jz)=0.0
	      zthzig(jz)=0.0
	      zthzrb(jz)=0.0
	      zthzkb(jz)=0.0
	      zdzthe(jz)=cfutz(iscrp2)/1.e4
	    endif
