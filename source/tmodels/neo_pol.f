...
c
        real x
        real, allocatable :: vtab(:), G(:)
cbate        real vtab(Ns),G(Ns)
c
        real nustara
        real y(Nmu)
c
        allocate ( vtab(Ns), G(Ns) )
c
...
c ,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,,
      use com_block
c
      real, allocatable :: Mab(:,:,:,:), Nab(:,:,:,:)
c
      integer :: iprint = 0     !control parameter for output
c
      allocate ( Mab(Ns,Ns,0:1,0:1), Nab(Ns,Ns,0:1,0:1) )
c
      do k = 1, Nx
	do i=1,Ns
          do j=1,Ns
...
c rap 06-jun-03 dimmensions of all arrays is changed
c rap 02-jun-03 kerr is changed to scalar;
c               dimensions of bwork is changed to [maxn x maxn] 
...
c
!cap	integer, parameter :: maxn = 10
c
        real, allocatable :: uq(:,:), awork(:,:), bwork(:,:)
c
...
c
        ierr = 0
        allocate ( uq(neq,neq), awork(neq,neq), bwork(neq, neq)
     &    , stat = ierr )
        if ( ierr .ne. 0 ) then
          write (*,*) 'uq, awork, and bwork not allocated in neo_pol.f'
          stop
        endif
c
	bwork = 0.0
c
        ierr = 0
        allocate ( ipiv(neq), stat = ierr )
        if ( ierr .ne. 0 ) then
          write (*,*) 'ipiv is not allocated in neo_pol.f'
          stop
        endif
c
	do k = 1, Nx
	  awork(1:neq,1:neq) = a(1:Nl*Ns,1:Nl*Ns,k)
	  bwork(1:neq,1:1) = ysr(1:Nl*Ns,k:k)
	  call matrx1(awork, neq, neq, ipiv, kerr)
	  call matrx2(uq, bwork, awork,
     &			ipiv, neq, neq, 1, neq, 1)
c
	  do i=1,Ns
	    Vp(i,k)=uq(2*i-1,1)*Bp(k)
	  end do
	end do
c
	if(iprint.eq.1) then
