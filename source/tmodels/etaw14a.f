...
!| 
c@etaw14a.f
c rap 31-jul-02 dimag function is changed to aimag
...
c
        write (nout,*)
        write (nout,*)
     &   'Weiland-Nordman eigenvalue equations, subroutine etaw14a'
        write (nout,*)
c
     ...
     &     ' Eigenvalues computed using IMSL routine gvcrg'
          ...
          write (nout,*)
     &     ' Eigenvalues computed using NAG14 routine'
        endif
c
        if ( ieq .eq. 11) then
          write (nout,*)
     ...
     &     ' Five eqns for e phi/T_e, T_H, n_H, T_e, and Vp'
        ...
        elseif ( ieq .eq. 2 ) then
          ...
          write (nout,*)
          write (nout,*) ieq,' = ieq in sbrtn etaw14a'
          call abortb(nout
     &     ,'the value of ieq is wrong in sbrtn etaw14a')
        endif
c
        inital = .false.
c
...
c
     ...
     & ,' neq .lt. 2 in sbrtn etaw14a')
c
      if ( ndim .gt. idim ) call abortb (nout
     & ,' ndim .gt. idim in sbrtn etaw14a')
c
c
c..initialize arrays
c
...
c
!| 
!| The matrix $ G_{i,j} = {\tt zgm(i,j)} $ consists of normalized
!| gradients
!| \[  G_{i,.} = ( \frac{ R \nabla T_H}{ T_H},
!| \; \frac{ R \nabla n_H}{ n_H},
!| \; \frac{ R \nabla T_e}{ T_e},
...
!| \; \frac{ R \nabla T_Z}{ T_Z },
!|  \cdots ) \]
!| The first column $G_{i,1}$ holds the input values of these ratios.
!| In subsequent columns $G_{i,j}$, each of these ratios is incremented
!| in turn while the others are set to the input values:
!| \[ G_{i,i} = G_{i,1} + d G. \]
!| 
c
...
c  Note:  zanorm = 1./kpc is used to normalize A_\parallel
c
        zanorm = cetain(11)
c
        bt  = 1.5
...
!| 
!| 
!| The diffusivity matrix is computed from the heat and particle fluxes
!| in the following way:
!| \[ F_i = \sum_{j=1}^{n} D_{i,j} G_j + v_i \]
!| where $ F_1 $ is the flux of $ n_H T_H $, $ F_2 $ is the flux of $ n_H $,
!| $ F_3 $ is the flux of $ n_e T_e $, $ F_4 $ is the flux of $ n_Z $, and
!| $ F_5 $ is the flux of $ n_Z T_Z $.
!| Here, G is an array of the following gradients:
!| \[ G = ( \frac{ R \nabla T_H}{ T_H},
!| \; \frac{ R \nabla n_H}{ n_H},
!| \; \frac{ R \nabla T_e}{ T_e},
!| \; \frac{ R \nabla n_Z}{ n_Z},
...
!|  \cdots ) \]
!| Then
!| \[ D_{i,j} = \Partial{ F_i }{ G_j} \]
!| and
!| \[ v_i = F_i - \sum_{j=2}^{n} D_{i,j} G_j. \]
!| 
c
...
c
        write (nout,*)
        write (nout,*) '--------------------------------------'
     &    ,'  Final results from sbrtn etaw14a'
c
        write (nout,*)
