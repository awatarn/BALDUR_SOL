...
c
     ...
     & , gamma,    chieff, chieffitg, chiefftem
     & , vflux,  difthi,  velthi
...
c
     ...
     & , tauzin,   fnzin,    czin,     azin,     fnsin,    betaein
     & , ftrapein, vef,      q,        shear,    ekyrhoin, wexb
     & , ndim,     omega,    gamma,    chieff,   chieffitg, chiefftem
     & , vflux,    nmodes,   nerr )
c
...
c
     ...
     & , omega,    gamma,   chieff,  chieffitg, chiefftem,  vfluxout 
c
...
c
     ...
     &   , zflxtem(idp), zflxitg(idp), zchitem(idp), zchiitg(idp)
c
...
c
c  zflxtem(j) = fluxes from TEM modes (ie, modes with zomega(j) > 0)
c  zflxitg(j) = fluxes from ITG modes (ie, modes with zomega(j) <= 0)
c  zchitem(j) = effective diffusivities from TEM modes
c  zchiitg(j) = effective diffusivities from ITG modes
c
      real 
...
c
c..separate fluxes for TEM and ITG modes
c
          if ( zomega(j) .gt. 0. ) then
            zflxtem(1) = zflxtem(1) + zflxph
          else
            zflxitg(1) = zflxitg(1) + zflxph
          endif
c
...
c
          if ( zomega(j) .gt. 0. ) then
            zflxtem(2) = zflxtem(2) + zflxnh
          else
            zflxitg(2) = zflxitg(2) + zflxnh
          endif
c
...
c
            if ( zomega(j) .gt. 0. ) then
              zflxtem(3) = zflxtem(3) + zflxpe
            else
              zflxitg(3) = zflxitg(3) + zflxpe
            endif
c
          endif
c
...
c
            if ( zomega(j) .gt. 0. ) then
              zflxtem(4) = zflxtem(4) + zflxnz
            else
              zflxitg(4) = zflxitg(4) + zflxnz
            endif
c
          endif
c
...
c
            if ( zomega(j) .gt. 0. ) then
              zflxtem(5) = zflxtem(5) + zflxpz
            else
              zflxitg(5) = zflxitg(5) + zflxpz
            endif
c
          endif
c			
...
c
      zchitem(1) = zflxtem(1) / sign( max( abs( zgth ), zepsqrt ),zgth)
      zchitem(2) = zflxtem(2) / sign( max( abs( zgnh ), zepsqrt ),zgnh)
      zchitem(3) = zflxtem(3) / sign( max( abs( zgte ), zepsqrt ),zgte)
      zchitem(4) = zflxtem(4) / sign( max( abs( zgnz ), zepsqrt ),zgnz)
      zchitem(5) = zflxtem(5) / sign( max( abs( zgtz ), zepsqrt ),zgtz)
c
      zchiitg(1) = zflxitg(1) / sign( max( abs( zgth ), zepsqrt ),zgth)
      zchiitg(2) = zflxitg(2) / sign( max( abs( zgnh ), zepsqrt ),zgnh)
      zchiitg(3) = zflxitg(3) / sign( max( abs( zgte ), zepsqrt ),zgte)
      zchiitg(4) = zflxitg(4) / sign( max( abs( zgnz ), zepsqrt ),zgnz)
      zchiitg(5) = zflxitg(5) / sign( max( abs( zgtz ), zepsqrt ),zgtz)
c
...
c
      do j1=1,min(ieq-1,5)
        ...
        chiefftem(j1) = zchitem(j1)
        chieffitg(j1) = zchiitg(j1)
      enddo
