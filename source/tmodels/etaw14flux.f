...
c
        write (nout,*)
        write (nout,*)
     &   'Weiland-Nordman eigenvalue equations, subroutine etaw14flux'
        write (nout,*)
c
          write (nout,*)
...
c
     ...
     &     ' Five eqns for e phi/T_e, T_H, n_H, T_e, and Vp'
        ...
        elseif ( ieq .eq. 2 ) then
          ...
          write(nout,*)'the value of ieq is wrong in sbrtn etaw14flux'
          nerr = 2
          return
        endif
c
...
c
      if ( neq .lt. 2 ) then
         write(nout,*) ' neq .lt. 2 in sbrtn etaw14flux'
         nerr = 2
         return
      else if ( ndim .gt. idim ) then
         write(nout,*) ' ndim .gt. idim in sbrtn etaw14flux'
         nerr = 2
         return
      endif
c
...
c        if (abs(zerrmax) .gt. 100*zepsqrt) nerr = max (nerr , 1)
        ...
        write (nout,*) nerr,
     &       ' = nerr, error in eigenvalue in etaw14flux'
      endif
...
c
      if ( ifail .gt. 0 ) then
        write(nout,*)
     &   'ifail .gt. 0 after call tomsqz in sbrtn etaw14flux'
        nerr = 2
        return
      endif
c
...
c
        write (nout,*)
        write (nout,*) '--------------------------------------'
     &    ,'  Final results from sbrtn etaw14flux'
c
        write (nout,*)
