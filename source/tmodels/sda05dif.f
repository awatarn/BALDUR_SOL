...
c
      ...
      difftz = -fluxtz / zgtz
c
c..compute diagonal elements and return
c
      if ( diffth .gt. zero ) then
        condif(1,1) = diffth
      else
        convel(1) = fluxth
      endif
c
      if ( diffnh .gt. zero ) then
        condif(2,2) = diffnh
      else
        convel(2) = fluxnh
      endif
c
      if ( diffte .gt. zero ) then
        condif(3,3) = diffte
      else
        convel(3) = fluxte
      endif
c
      if ( diffnz .gt. zero ) then
        condif(4,4) = diffnz
      else
        convel(4) = fluxnz
      endif
c
      if ( difftz .gt. zero ) then
        condif(5,5) = difftz
      else
        convel(5) = fluxtz
      endif
c
c..return after computing effective diffusivities (temporary choice)
c
      if ( ndim .lt. 99 ) return
c
c..finite differences wrt normalized gradients
c
      zdgth = 0.1
      zdgnh = 0.1
      zdgte = 0.1
