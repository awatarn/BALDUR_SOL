...
!| 0tverb < mixed_merba.tex > mixed_merba.f
!| %
...
c
        ...
        zkyrho = 0.316
c
c..declare correction coeficient
c
c
c     define the jz value at 80 0f normalized radius
c
...
c
      ...
      zcoef4   = 1.25
      switch   = 0
      zdte     = abs(tekev(npoints1)-tekev(npoints))/tekev(npoints)
      zrlpe    = zgte + zgne
c
c
c
...
c
c        zgamma = 3.0959e6 * sqrt(zte)/(zq*zrmaj)
c
...
c
      call mixerba(
     &   zshear,      zwexb,      zrmaj,      cswitch(21)
     & , chibohm,     zq,         zrlpe,      zte,    zti
     & , gradte,      zra,        zdte,       zbtor
...
c
         thdmix(jz)  = fig(1) * dh
         themix(jz)  = fig(2) * chie
         thimix(jz)  = fig(3) * chii
         thzmix(jz)  = fig(4) * dimp
c
c  put value of gbohm and bohm terms into kb and rb terms for
c  diagnostic purposes
c
c
         thebohm(jz) = fig(2) * chbe
         thibohm(jz) = fig(3) * chbi
c
         thegb(jz)   = fig(2) * chgbe
         thigb(jz)   = fig(3) * chgbi
c
