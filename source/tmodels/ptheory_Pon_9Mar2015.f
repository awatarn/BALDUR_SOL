...
c  lthery(48) = 7     using Neoclassical Rotations calculation
c
...
c
         else if (lthery(48) .eq. 7) then
c..neoclassical rotations  model
            aspect_NTV = zrminor(mzones)/zrmajor(1)
            fc_NTV = 1.-1.46*sqrt(aspect_NTV)+0.46*aspect_NTV*
     &      sqrt(aspect_NTV)
            g_NTV = (1.-fc_NTV)/fc_NTV
...
c
      write (6,*)'Cime', tai*uist,ztaue,zftaug,ztauxg,zfiter,ziter
     &  ,zftoh,ztoh
...
c                
     ...
     &            + cdprof(jk) ) * bpoli(jk) / (2 * zdensi(jk)) 
       vtorr(jk) = vtorr(jk) + boyle(jk) 
c EFDA–JET–PR(08)10      
c       vtorLH(jk)=RNPLH/c
c       vtorr(jk) = vtorr(jk) 
      write(6,*) 'ion telocity', boyle(jk)   
          end do
...
c        write(6,*) 'ion telocity', boyle(mj)     
     ...
     &              mu2_NTV*mu2_NTV
               K1_NTV = (sqrt(2.)+alpha_NTV)*mu2_NTV/D_NTV
c               zvrotxb(jz) = K1_NTV*
c     &              zgrdti(jz)*ztikev(jz)*1000./(zrmajor(1)*zcharge(2)*
c     &              bpoli(jz))
c               zvrotxb(jz) = vtorr(mj)
                zvrotxb(jz) = vtorr(jz)
            end do
            write(nprint,*) 'Predicted wexb  by Neoclassical 
     &              Rotation Model'
c
         end if
c
