...
************************************************************************
     ...
     > zrho,           ! 0:jmaxm 0 < rho < 1 normalized toroidal flux (rho=rho/rho(a))
...
! OPTIONAL
     ...
     > zpte_m,         ! externally provided log gradient te w.r.t rho (i_grad=2)
     > zpti_m,         ! externally provided log gradient ti w.r.t rho
     > zpne_m,         ! externally provided log gradient ne w.r.t rho
     > zpni_m,         ! externally provided log gradient ni w.r.t rho
     > zdiffnem,       ! ion plasma diffusivity in m**2/sec
     > zchietem,       ! electron ENERGY diffuivity in m**2/sec
     > zchiitim,       ! ion      ENERGY diffuivity in m**2/sec
     > zchieetg_m,     ! electron diffusivity from ETG modes, [m^2/sec]
     > zetaphim,       ! toroidal velocity diffusivity in m**2/sec
     > zetaparm,       ! parallel velocity diffusivity in m**2/sec
     > zetaperm,       ! perpendicular velocity diffusivity in m**2/sec
     > zexchm,         ! turbulent electron to ion ENERGY exchange in MW/m**3
     > zegamma_m,      !0:jmaxm exb shear rate in units of local csda_m
     > zegamma_d,      !0:jmaxm exb shear rate delayed by i_delay steps
     > zgamma_p_m,     !0:jmaxm par. vel. shear rate in units of local csda_m
     > zanrate_m,      !0:jmaxm leading mode rate in unints of local csda_m
     > zanrate2_m,     !0:jmaxm 2nd mode rate in units of local csda_m
     > zanfreq_m,      !0:jmaxm leading mode frequency
     > zanfreq2_m,     !0:jmaxm 2nd mode frequency
     > ierr
     >  )
c************************************************************************
...
c.......begin dimensions.................
      ...
      real*8 epsilon, zpi
      parameter ( epsilon = 1.D-34 )
 
c external arrays

      integer, intent(in)           :: jmaxm
      integer, intent(in)           :: jshoot
      integer, intent(in)           :: jmm
      integer, intent(in)           :: i_grad
      integer, intent(in)           :: idengrad
      integer, intent(in)           :: itport_pt(1:5)
      integer, intent(in)           :: i_delay
      integer, intent(in)           :: irotstab
      integer, intent(in)           :: iglf
      real*8, intent(in)            :: alpha_e
      real*8, intent(in)            :: x_alpha
      real*8, intent(out), optional :: zdiffnem
      real*8, intent(out), optional :: zchietem
      real*8, intent(out), optional :: zchiitim
      real*8, intent(out), optional :: zetaphim
      real*8, intent(out), optional :: zetaparm
      real*8, intent(out), optional :: zetaperm
      real*8, intent(out), optional :: zexchm
      real*8, intent(in)            :: rmajor_exp
      real*8, intent(in)            :: zimp_exp
      real*8, intent(in)            :: amassimp_exp
      real*8, intent(in)            :: bt_exp
      real*8, intent(in)            :: arho_exp
      real*8, intent(in)            :: amassgas_exp
      real*8, intent(in)            :: te_m(0:jmaxm)
      real*8, intent(in)            :: ti_m(0:jmaxm)
      real*8, intent(in)            :: ne_m(0:jmaxm)
      real*8, intent(in)            :: ni_m(0:jmaxm)
      real*8, intent(in)            :: ns_m(0:jmaxm)
      real*8, intent(in)            :: angrotp_exp(0:jmaxm)
      real*8, intent(in)            :: egamma_exp(0:jmaxm)
      real*8, intent(in)            :: gamma_p_exp(0:jmaxm)
      real*8, intent(inout)         :: vphi_m(0:jmaxm)
      real*8, intent(inout)         :: vpar_m(0:jmaxm)
      real*8, intent(inout)         :: vper_m(0:jmaxm)
      real*8, intent(in)            :: zrho(0:jmaxm)
      real*8, intent(in)            :: rmin_exp(0:jmaxm)
      real*8, intent(in)            :: rmaj_exp(0:jmaxm)
      real*8, intent(in)            :: gradrho_exp(0:jmaxm)
      real*8, intent(in)            :: gradrhosq_exp(0:jmaxm)
      real*8, intent(in)            :: zeff_exp(0:jmaxm)
      real*8, intent(in)            :: q_exp(0:jmaxm)
      real*8, intent(in)            :: shat_exp(0:jmaxm)
      real*8                        :: bteff_exp(0:jmaxm)
      real*8, intent(in)            :: alpha_exp(0:jmaxm)
      real*8, intent(in)            :: elong_exp(0:jmaxm)
      real*8, intent(out)           :: diff_m(0:jmaxm)
      real*8, intent(out)           :: chie_m(0:jmaxm)
      real*8, intent(out)           :: chii_m(0:jmaxm)
      real*8, intent(out), optional :: zchieetg_m(0:jmaxm)
      real*8, intent(out)           :: etaphi_m(0:jmaxm)
      real*8, intent(out)           :: etapar_m(0:jmaxm)
      real*8, intent(out)           :: etaper_m(0:jmaxm)
      real*8, intent(out)           :: exch_m(0:jmaxm)
      real*8, intent(out), optional :: zegamma_m(0:jmaxm)
      real*8, intent(out), optional :: zegamma_d(0:jmaxm,10)
      real*8, intent(out), optional :: zgamma_p_m(0:jmaxm)
      real*8, intent(out), optional :: zanrate_m(0:jmaxm)
      real*8, intent(out), optional :: zanrate2_m(0:jmaxm)
      real*8, intent(out), optional :: zanfreq_m(0:jmaxm)
      real*8, intent(out), optional :: zanfreq2_m(0:jmaxm)
      integer, intent(out),optional :: ierr
      !
      integer j, jin, jout, jm,
...
c internal arrays (which can be converted to externals)
      ...
      real*8, intent(inout), optional :: zpte_m(0:jmaxm)
      real*8, intent(inout), optional :: zpti_m(0:jmaxm)
      real*8, intent(inout), optional :: zpne_m(0:jmaxm)
      real*8, intent(inout), optional :: zpni_m(0:jmaxm)
      real*8, intent(in), optional  :: zpte_in
      real*8, intent(in), optional  :: zpti_in
      real*8, intent(in), optional  :: zpne_in
      real*8, intent(in), optional  :: zpni_in
      real*8
     ...
     >  alpha_m(0:jmaxm),vstarp_m(0:jmaxm)
      ...
      real*8 :: etaphim, etaparm, etaperm, exchm
      real*8, allocatable :: rho(:)
      real*8, allocatable :: chieetg_m(:)
      real*8, allocatable :: egamma_m(:)
      real*8, allocatable :: egamma_d(:,:)
      real*8, allocatable :: gamma_p_m(:)
      real*8, allocatable :: anrate_m(:)
      real*8, allocatable :: anrate2_m(:)
      real*8, allocatable :: anfreq_m(:)
      real*8, allocatable :: anfreq2_m(:)
      real*8, allocatable :: zzpte_m(:), zzpti_m(:)
     >        , zzpne_m(:), zzpni_m(:)

c working arrays and variables

      real*8, allocatable :: ve(:),vpar(:)
...
c some internals
      ...
      real*8 tem,tim,nem,nim,nsm,zeffm, aiwt_jp1, xnimp_jp1, xnimp
      real*8 vnewk3x
...
c
      allocate(rho(0:jmaxm), chieetg_m(0:jmaxm), egamma_m(0:jmaxm)
     >       , egamma_d(0:jmaxm,10), gamma_p_m(0:jmaxm)
     >       , anrate_m(0:jmaxm), anrate2_m(0:jmaxm)
     >       , anfreq_m(0:jmaxm), anfreq2_m(0:jmaxm)
     >       , zzpte_m(0:jmaxm), zzpti_m(0:jmaxm)
     >       , zzpne_m(0:jmaxm), zzpni_m(0:jmaxm)
     >       , ve(0:jmaxm),vpar(0:jmaxm) )
c
      betae_m     = 0.D0
      xnu_m       = 0.D0
      cgyrobohm_m = 0.D0
...
c      vphi_m(j)      = 0.D0
      ...
      rho         = zrho
      !
      if (present(zpte_m)) zzpte_m = zpte_m
      if (present(zpti_m)) zzpti_m = zpti_m
      if (present(zpne_m)) zzpne_m = zpne_m
      if (present(zpni_m)) zzpni_m = zpni_m
c
...
c
      if ( i_grad == 0 ) then
        do j=jm-jptf, jm+jpt
          ...
          zzpte_m(j)=-(dlog(te_m(j-1))-dlog(te_m(j)))/drho
          zzpti_m(j)=-(dlog(ti_m(j-1))-dlog(ti_m(j)))/drho
          zzpne_m(j)=-(dlog(ne_m(j-1))-dlog(ne_m(j)))/drho
          zzpni_m(j)=-(dlog(ni_m(j-1))-dlog(ni_m(j)))/drho
        enddo
      endif
      if (i_grad /= 1) then
        zpmte=zzpte_m(jm+1-jshoot)
        zpmti=zzpti_m(jm+1-jshoot)
        zpmne=zzpne_m(jm+1-jshoot)
        zpmni=zzpni_m(jm+1-jshoot)
      else
        if ((present(zpte_in)).and.(present(zpti_in)).and.
     >      (present(zpne_in)).and.(present(zpni_in))) then
          zpmte=zpte_in
          zpmti=zpti_in
          zpmne=zpne_in
          zpmni=zpni_in
...
cx        angrotp_exp(j)=corot*angrotp_exp(j)
     ...
     > (zzpni_m(j)+alpha_neo*zzpti_m(j))*vstar_sign*pgeo_local
     > -rho(j)*rdrho_local*
     ...
     >((alpha_neo-1.D0)*zzpti_m(j))*rho(j)*rdrho_local*
...
c     >    csda_m(j)*arho_exp*rhosda_m(j)
     ...
     >   (zzpni_m(j)+zzpti_m(j))*vstar_sign*pgeo_local
       endif
...
c
      ...
      chie_m(jm)=chietem
      chii_m(jm)=chiitim
      chieetg_m(jm)=cmodel*gfac*chie_e_gf*cgyrobohm_m(jm)
...
c
       if (present(zdiffnem))   zdiffnem = diffnem
       if (present(zchietem))   zchietem = chietem
       if (present(zchiitim))   zchiitim = chiitim
       if (present(zetaphim))   zetaphim = etaphim
       if (present(zetaparm))   zetaparm = etaparm
       if (present(zetaperm))   zetaperm = etaperm
       if (present(zexchm))     zexchm   = exchm
       if (present(zchieetg_m)) zchieetg_m = chieetg_m
       if (present(zegamma_m))  zegamma_m  = egamma_m
       if (present(zegamma_d))  zegamma_d  = egamma_d
       if (present(zgamma_p_m)) zgamma_p_m = gamma_p_m
       if (present(zanrate_m))  zanrate_m  = anrate_m
       if (present(zanrate2_m)) zanrate2_m = anrate2_m
       if (present(zanfreq_m))  zanfreq_m  = anfreq_m
       if (present(zanfreq2_m)) zanfreq2_m = anfreq2_m
       deallocate(rho, chieetg_m, egamma_m
     >       , egamma_d, gamma_p_m, anrate_m, anrate2_m
     >       , anfreq_m, anfreq2_m, zzpte_m, zzpti_m
     >       , zzpne_m, zzpni_m, ve, vpar )

       return
       end subroutine callglf2dp
c
