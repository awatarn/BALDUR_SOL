*/ 12:00 08-aug-88  11040 bald88/wbaldn1  wsutil  DODE
*/ Shampine and Gordon ode solver for use with the saturated tearing
*/ mode code DISLAND.
*select printlog=log
*file name=tode
c--------1---------2---------3---------4---------5---------6---------7-c
...
c
     ...
     1  itold,idelsn/1,13,25,38,50,62,75,76,88,89,90,91,92,93/
      ...
      nornd = iwork(2) .ne. -1
    1 call de0000(f,neqn,y,t,tout,relerr,abserr,iflag,work(iyy),
     ...
     5  iwork(5))
      ...
      subroutine de0000(f,neqn,y,t,tout,relerr,abserr,iflag,
...
c
c       modified on 28th june 1978 by p.w.gaffney
c
        data mentry/1/
        if(mentry.ne.1) goto 5
c
c       if first entry then compute  u and fouru
c
        call machin(u)
        fouru = 4.0*u
        mentry = 2
...
c
   50 if(abs(x-t) .lt. absdel) go to 60
      call intrp0(x,yy,tout,y,ypout,neqn,kold,phi,psi)
      iflag = 2
      t = tout
      told = t
      isnold = isn
...
c
  ...
  110   wt(l) = releps*abs(yy(l)) + abseps
      call step00(x,yy,f,neqn,h,eps,wt,start,
...
c
  130 nostep = nostep + 1
      ...
      go to 50
        ...
        subroutine step00(x,y,f,neqn,h,eps,wt,start,
     1  hold,k,kold,crash,phi,p,yp,psi,
     2  alpha,beta,sig,v,w,g,phase1,ns,nornd)
c
...
c
        data mentry/1/
        if(mentry.ne.1) goto 2
c
c       if first entry then compute  u,twou,and fouru
c
        call machin(u)
        twou = 2.0*u
        fouru= 4.0*u
...
c       ***     end block 4     ***
      ...
      subroutine intrp0(x,y,xout,yout,ypout,neqn,kold,phi,psi)
c
...
c--------1---------2---------3---------4---------5---------6---------7-c
*civic i=tode,b=b0,l=lode,lang=cft
