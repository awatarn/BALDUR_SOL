c--------1---------2---------3---------4---------5---------6---------7-c
c@synch  /11040/baldur/code/bald/dimprad.f
c  rgb 19-sep-90 18.66 Changed sbrtn EDITOR to sbrtn SYEDIT
c      and removed function zncnt, which was not being used
cahk
c subroutines required in Englade's calculation of synch radiation
      subroutine synch
      external case20
c
...
c     returned via the common block "INSINK".
c
c
c
c        Input variables:
...
c        temperature. =omega**2/((2.*pi)**3*c**2)
c
...
c
      call syopac(nzons,teqp,bavg,ndbug)
      taue=0.0
      tauo=0.0
      nze=nzons
      nzo=nzons
...
c
      do 150 ii=1,nzons
      nz=nzons-ii+1
      wmult=6.03e-12*deny(nz)/bavg(nz)
...
c
  ...
  160 continue
      ...
      ts=4.*voq(nz)*alphae(nz)
      xe=xe+ts
      srce=srce+ts*teqp(nz)
  170 continue
  175 continue
      if(nzo1.gt.nzons) go to 185
      do 180 nz=nzo1,nzons
      ts=4.*voq(nz)*alphao(nz)
      xo=xo+ts
      srco=srco+ts*teqp(nz)
  180 continue
  185 continue
c
...
c
      ...
      ts=4.*pi*alphae(nz)*(blackb*teqp(nz)-eray)*wght
      ploss(nz)=ploss(nz)+ts
  200 continue
  205 continue
      if(nzo1.gt.nzons) go to 225
      do 220 nz=nzo1,nzons
      ts=4.*pi*alphao(nz)*(blackb*teqp(nz)-oray)*wght
      ploss(nz)=ploss(nz)+ts
  220 continue
  225 continue
c
...
c
      tse=pi*areae*(blackb*teqp(nze)-eray)
      tso=pi*areao*(blackb*teqp(nzo)-oray)
      ptot=0.0
      do 350 nz=1,nzons
      if(nz.lt.nze1) ploss(nz)=ploss(nz)+tse*wght*deny(nz)/emass
      if(nz.lt.nzo1) ploss(nz)=ploss(nz)+tso*wght*deny(nz)/omass
      ptot=ptot+ploss(nz)*voq(nz)
  350 continue
     ...
     $   pwall,ptot
 9010    format(1x,'  synch edit....nw= ',i5//
     $   ,3x,'nze',2x,'nzo',5x,'wfreq',6x,'wght',4x,'blackb',5x,'areae'
     $   ,5x,'areao',
     $   /,1x,2i5,5e10.2,//6x,'emass',5x,'omass',7x,'qee',7x,'qoo',
     $   7x,'qoe',7x,'qeo',/1x,6e10.2//9x,'xe',8x,'xo',6x,'srce',6x,
     $   'srco',4x,'  eray',4x,'  oray',/1x,6e10.2//,
     $   10x,'pwall',11x,'ptot',/2e15.5)
         write(6,9015)
 9015    format(1x,81(1h-),/
     $   4x,'nz',6x,'dens',7x,'vol','    alphae','    alphao',5x,
     $   'ploss',/1x,81('-'))
         do 800 nz=1,nzons
         write(6,9020)nz,deny(nz),voq(nz),alphae(nz),alphao(nz),
     $   ploss(nz)
  800    continue
