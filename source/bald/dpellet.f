...
c
     ...
     r  zcosth(kmhrm), zpdent(mj), znorm(mj)
c
...
c         zstdev = standar deviation in Gaussian distribution
c         coeffG = the coefficient in front of Gassian distribution
c         znpelt = number of particles in one pellet
c         znorm(mj) = Gaussian distribution function
cpub c1-13 = coefficents for grdBdr
        real    dgbdr, zstdev, zarea, coeffG, znpelt
...
c
 ...
 304  continue      
c
cpub1 the pellet mass deposition profile has Gaussian distribution
      call scopy(izones,zpden,1,zpdent,1)
      znorm(1) = 0
      if (ypa(20) .lt. 99) then
	zstdev = ypa(20)
      else if (ypa(20) .ge. 99) then
        zstdev = ledge-dgbdr
      endif
      zarea = 0
c..determine number of particles in the pellet
      znpelt = 0
      dgbdr = dgbdr*(mzones-2)
      do 351 jz=lcentr,izones
	znpelt = znpelt + zdvol(jz)*zpden(jz) + zdvol(jz)*zpdenc(jz)
	znorm(jz) = exp(-((jz-(ledge-dgbdr))**2)/(2*zstdev*zstdev))
 351  continue
      do 352 jz=lcentr,izones
	zarea = zarea + znorm(jz)*zdvol(jz)
 352  continue
       write(nout,2006) dgbdi, zstdev, znpelt,lcentr,izones,zarea,
     # dgbdr
 2006 format(//,'dgbdi = ',1i4,/,'zstdev = ',1pe10.3,/,
     #'znpelt = ',1pe10.3,/,'lcentr = ',1i4,/,'izones =',1i4,
     #/,'zarea = ',1pe10.3,/,'dgbdr = ',1pe10.3) 
      write(*,*) 'The pellet deposition profile: '
      write(*,*) 'jz  znorm(jz)  zdvol(jz)  zpdent(jz)  chi(iih,jz)'
      do 353 jz=lcentr,izones
	znorm(jz) = znorm(jz)*(znpelt/zarea)
	zpden(jz) = znorm(jz)
	write(*,*) jz,'  ',znorm(jz),'  ',zdvol(jz),'  ',zpdent(jz),
     #' ',chi(iih,jz)
 353  continue
c 
cbpu2
...
c
      write(*,*) 'jz  chi-old(iih,jz)  chi-new(iih,jz)'
      if (iih .gt. 0) then
      do 306 jz=lcentr,ledge
      write(*,*) jz,chi(iih,jz),(1.0-frcor(it))*(zpden(jz)-zpdenc(jz))
...
c
cpub  the ionization losses are calucated at the ablation region
      call scopy(izones,zpdent,1,zpden,1)
c      
      write(*,*) 'jz  chi-old(lelec,jz)  chi-new(lelec,jz)'
      do 310 jz=lcentr,ledge
      write(*,*) jz,chi(lelec,jz),-z02*zpden(jz)+chi(lelec,jz)
      chi(lelec,jz)=-z02*zpden(jz)+chi(lelec,jz)
 310  continue
        chi(lelec,1)=chi(lelec,lcentr)
        call scopy(izones,znorm,1,zpden,1)
...
cpub T at the NEW cell will be lowered due to the ionization            
            tenew=(den(i)*te(i)-pden(i)*(2.0/3.0)*eion)/(den(i)+pden(i))
