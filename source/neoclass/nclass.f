     ...
     &                  bsjbp_s,bsjbt_s,chi_s,dn_s,gfl_s,qfl_s,sqz_s,
     &                  upar_s,utheta_s,vnnt_s,vneb_s,vnex_s,vqnt_s,
     &                  vqeb_s,vqex_s,xi_s,ymu_s,chip_ss,chit_ss,dp_ss,
     &                  dt_ss,iflag,message)
!***********************************************************************
...
!  p_fm(3)-poloidal moments of drift factor for PS viscosity (/m**2)
...
!  fex_iz(3,i,z)-moments of external parallel force on i,z (T*n/m**3)
...
!  chi_s(s)-heat conduction coefficient (diag comp) of s (rho**2/s)
...
!                  m=2, <E.B>
!                  m=3, fex_iz
!  vnnt_s(s)-convection velocity (off diag comps-p', T') of s (rho/s)
!  vneb_s(s)-<E.B> particle convection velocity of s (rho/s)
!  vnex_s(s)-external force particle convection velocity of s (rho/s)
!  vqnt_s(s)-conduction velocity (off diag comps-p', T') of s (rho/s)
!  vqeb_s(s)-<E.B> heat convection velocity of s (rho/s)
!  vqex_s(s)-external force heat convection velocity of s (rho/s)
...
!Declaration of output variables
     ...
     &               sqz_s(mx_ms),            upar_s(3,3,mx_ms),
     &               utheta_s(3,3,mx_ms),     vneb_s(mx_ms),
     &               vnex_s(mx_ms),           vnnt_s(mx_ms),
     &               vqeb_s(mx_ms),           vqex_s(mx_ms),
     &               vqnt_s(mx_ms),           xi_s(mx_ms),
     &               ymu_s(3,3,mx_ms)
...
!     Potato orbit contribution to viscosity
     ...
     &   .and.(k_potato.ne.0)) THEN
      ...
      ELSE
        k_potato=0
        iflag=-1
        message='NCLASS(5)/WARNING:no potato orbit viscosity'
      ENDIF
!     Pfirsch-Schluter contribution to viscosity
      ...
      ELSE
        k_pfirsch=0
        iflag=-1
        message='NCLASS(6)/WARNING:no Pfirsch-Schluter viscosity'
      ENDIF
!     Banana contribution to viscosity
      ...
      ELSE
        k_banana=0
        iflag=-1
        message='NCLASS(7)/WARNING:no banana viscosity'
      ENDIF
!     No viscsoity
      IF((k_banana.eq.0).and.(k_pfirsch.eq.0)) THEN
        k_potato=0
        iflag=-1
        message='NCLASS(8)/WARNING:no viscosity'
      ENDIF
...
!           Set isotope number and charge state for this species
      ...
      IF(m_s.lt.2.or.m_s.gt.mx_ms) THEN
        iflag=1
        message='NCLASS(9)/ERROR:m_s must be ge 2 and le mx_ns'
        GOTO 1000
      ENDIF
...
!Calculate species charge state density factor, total nT, squeezing
     ...
     &               *p_gr2phi/(z_coulomb*iz)
        IF(sqz_s(i).gt.10.0) THEN
          sqz_s(i)=10.0
        ELSEIF(sqz_s(i).lt.0.5) THEN
          sqz_s(i)=0.5
        ENDIF
      ENDDO
...
!Get normalized parallel flows within a surface
     ...
     &                 chit_ss,dp_ss,dt_ss,iflag,message)
...
!  Full coefficient matrices 
      DO i=1,m_s
        im=jm_s(i)
        iza=IABS(jz_s(i))
!  Diagonal diffusivity and conductivity
        dn_s(i)=dp_ss(i,i)
        chi_s(i)=chip_ss(i,i)+chit_ss(i,i)
!  Off-diagonal expressed as radial velocity
        vnnt_s(i)=(gfl_s(1,i)+gfl_s(2,i)+gfl_s(3,i)
     &          +dn_s(i)*(grp_iz(im,iza)-den_iz(im,iza)*grt_i(im))
     &          /temp_i(im))/den_iz(im,iza)
        vqnt_s(i)=((qfl_s(1,i)+qfl_s(2,i)+qfl_s(3,i))/z_j7kv
     &          /den_iz(im,iza)+chi_s(i)*grt_i(im))/temp_i(im)
!  <E.B> particle and heat convection velocities
        vneb_s(i)=gfl_s(4,i)/den_iz(im,iza)
        vqeb_s(i)=qfl_s(4,i)/den_iz(im,iza)/temp_i(im)/z_j7kv
!  External force particle and heat convection velocities
        vnex_s(i)=gfl_s(5,i)/den_iz(im,iza)
        vqex_s(i)=qfl_s(5,i)/den_iz(im,iza)/temp_i(im)/z_j7kv
      ENDDO
 1000 RETURN
      END SUBROUTINE
...
!  Ions
c            xlnab(im,jm)=40.3
c     &                   -ALOG(xz(im)*xz(jm)*(amu_i(im)+amu_i(jm))
c     &                   /(amu_i(im)*temp_i(jm)+amu_i(jm)*temp_i(im))
c     &                   *SQRT(xnz2(im)/temp_i(im)+xnz2(jm)/temp_i(jm)))
            xlnab(im,jm)=17.0
          ENDIF
        ENDDO
      ENDDO
